...
int main(void)
{
	...
	if(strlen(a_down) >= strlen(b_down))
	{
		for(int i=0 ; i<strlen(a_down) ; i++){
			if(b_down[i] == 0)
				...
				plus_down[i] += (a_down[i]-48) + (b_up[i]-48);
	...
	if(strlen(a_up) >= strlen(b_up)) // 정수부(a_up길이 >= b_up의 길이) 덧셈 후 plus_up 배열에 저장 
	{
	...
	else // 정수부(a_up의 길이 < b_up의 길이) 덧셈 후 plus_up 배열에 저장
	{
		...
		if(plus_up[strlen(b_up)] != 0)
		{
			for(int i = strlen(b_up) ; i>=0 ; i--)
				printf("%d\n", plus_up[i]);
		}
		else
